            ...
            if k in ['samples', 'inputs']:
                ...
                if config.model.params.cond_stage_key == 'segmentation':
                    xyz, rgb = custom_to_pcd(
                        x,
                        config,
                        logs['original_conditioning'][i]
                    )
                else:
                    xyz, rgb = custom_to_pcd(x, config)

                # ----------------------------
                # Save TXT (existing behaviour)
                # ----------------------------
                txt_path = os.path.join(
                    pcdlogdir,
                    f"{k}_{n_saved:06}.txt"
                )
                ...
                points = np.empty((xyz.shape[0], 4), dtype=np.float32)
                points[:, :3] = xyz
                points[:, 3] = depth.astype(np.float32)

                bin_path = os.path.join(
                    pcdlogdir,
                    f"{k}_{n_saved:06}.bin"
                )
...
if __name__ == "__main__":
    ...
    resume_exists = opt.resume is not None and os.path.exists(opt.resume)
    file_exists   = opt.file is not None and os.path.exists(opt.file)

    print("opt.resume =", opt.resume)
    print("resume_exists =", resume_exists)
    print("cwd =", os.getcwd())
    
    if not resume_exists and not file_exists:
    ...
    if os.path.isfile(opt.resume):
        ...
        if logdir.endswith("checkpoints"):
