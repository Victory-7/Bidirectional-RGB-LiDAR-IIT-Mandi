...
class KITTIBase(DatasetBase):
    ...
    def __getitem__(self, idx):
        ...
        # data_path is already the .bin file
        ...
        example["image"] = proj_range
        example["mask"] = proj_mask

        if self.return_pcd:
            reproj_sweep, _, _ = range2pcd(
                proj_range[0] * .5 + .5,
                self.fov,
                self.depth_range,
                self.depth_scale,
        ...
        if self.degradation_transform:
            example["degraded_image"] = self.degradation_transform(proj_range)

        if self.condition_key == "camera":
            cameras = self.load_camera(data_path)
            example["camera"] = cameras
...
class SemanticKITTIBase(KITTIBase):
    ...
    def prepare_data(self):
        ...
        self.data = sorted(
            glob.glob(
                "/home/teaching/Suhani/project/kitti_object/testing/image_2/*.png"
            )
        )

        print("Found", len(self.data), "test RGB images")
...
class KITTI360Base(KITTIBase):
    ...
    def prepare_data(self):
        ...
        self.data = sorted(
            glob.glob(
                "/home/teaching/Suhani/project/kitti_object/testing/velodyne/*.bin"
            )
        )

        print("Found", len(self.data), "lidar scans")
    ...
    def load_camera(self, path):
        ...
        camera = camera.resize((1024, 64))

        camera = np.array(camera).astype(np.float32) / 255.
        camera = camera.transpose(2, 0, 1)

        if self.view_transform:
            camera = self.view_transform(camera)

        camera_list = np.split(
            camera,
            self.split_per_view,
            axis=2
        )
...
class KITTIImageBase(KITTIBase):
    ...
    def prepare_data(self):
        ...
        print(f"DEBUG {self.split}: {len(self.data)} images loaded")

        if len(self.data) > 0:
            print("DEBUG first image:", self.data[0])
