...
try:
    ...
    class LightningDeprecationWarning(Warning):
        pass
...
class SetupCallback(Callback):
    ...
    def oon_fit_start(self, trainer, pl_module):
        if trainer.global_rank == 0:
...
class CUDACallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module, outputs=None):
        ...
        try:
            rank_zero_info(
                f"Average Epoch time: {epoch_time:.2f} seconds"
            )
            rank_zero_info(
                f"Average Peak memory {max_memory:.2f} MiB"
            )
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    try:
        ...
        # default to ddp
        # GPU configuration for Lightning >=1.8
